...
class TestRecursiveReplace(unittest.TestCase):
    ...
    def test_recursive_replace_no_match(self):
        data = {"a": 1, "b": {"c": 2, "d": {"e": 3, "f": 4}}}
        expected = data.copy()
...
class TestAddItem(unittest.TestCase):
    ...
    def test_add_item_to_nested_dict(self):
        data = {"a": {"b": {"c": 1}}}
        expected = {"a": {"b": {"c": 1, "d": "new_value"}}}
        result = add_item(data, "a.b.d", "new_value")
        self.assertEqual(result, expected)

    def test_add_item_creates_nested_dict(self):
        data = {}
        expected = {"a": {"b": {"c": "new_value"}}}
        result = add_item(data, "a.b.c", "new_value")
        self.assertEqual(result, expected)

    def test_add_item_overwrites_existing_value(self):
        data = {"a": {"b": {"c": 1}}}
        expected = {"a": {"b": {"c": "new_value"}}}
        result = add_item(data, "a.b.c", "new_value")
        self.assertEqual(result, expected)
